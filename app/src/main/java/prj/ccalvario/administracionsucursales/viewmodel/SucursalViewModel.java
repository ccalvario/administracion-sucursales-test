...
public class SucursalViewModel extends AndroidViewModel {
    ...
    public void saveSucursal() {
        ...
        sucursal.setNumero(Integer.parseInt(numero.get()));
        sucursal.setCodigoPostal(Integer.parseInt(codigoPostal.get()));
        sucursal.setCiudad(ciudad.get());
        sucursal.setPais(pais.get());
        Log.d("saveSucursal nombre " + nombre.get() + " calle " + calle.get());
        insert(sucursal);
    }
